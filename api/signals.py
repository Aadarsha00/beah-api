...
def contact_message_notification(sender, instance, created, **kwargs):
    ...
    if created:
        try:
            ...
            <p><strong>Phone:</strong> {instance.phone or 'Not provided'}</p>
            ...
            # Log the error but don't raise it to avoid breaking the request
            import logging

            logger = logging.getLogger(__name__)
            logger.error(f"Failed to send contact message notification: {e}")
...
def promotion_status_change(sender, instance, created, **kwargs):
    ...
    if not created:  # Only for updates
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.error(f"Error in promotion status change signal: {e}")
    ...
    if created and instance.is_important:
        try:
            ...
            if admin_emails:
                ...
                message = f"""
                An important admin note has been created by {instance.created_by.get_full_name() or instance.created_by.username}.
            ...
            logger = logging.getLogger(__name__)
            logger.error(f"Failed to send important note notification: {e}")
